...
inline auto generate_pointparblob(int nbre_point_blob,int nbre_blob,double bound_min,double bound_max,double centre_x,double centre_y){
  auto start = std::chrono::system_clock::now();
...
int main(int argc,char *argv[]){
  ...
  auto points = make_uniform_dataset(-10,10,80);
  ...
  auto g = generate_pointparblob(tab.size()/K,K,50,100,m.first[0],m.second[0]);
  auto g1 = generate_pointparblob(tab.size()/K,K,-30,30,m.first[1],m.second[1]);
  auto g2 = generate_pointparblob(tab.size()/K,K,30,50,m.first[2],m.second[2]);

  plt::plot(g.first,g.second,"o");
  plt::plot(g1.first,g1.second,"o");
  plt::plot(g2.first,g2.second,"o");
  plt::plot(m.first,m.second,"o");
  plt::scatter(m.first,m.second);
  plt::show();
